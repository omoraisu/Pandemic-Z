    ...
    while [i < length row] [
      ...
      ask patch x y [
        ...
        if char = "O" [
          ...
          set shelter-integrity 100
          set max-break-time (shelter-integrity / 2) + 30  ; 75-100 ticks to break in
          set being-broken? false
          set break-timer 0
        ]
      ]
...
;  if state = "wandering"
  ...
  if (energy < 20) and state != "hiding" and state != "running" [
    ; Look for safe place to rest
    if any? patches in-radius vision-range with [pcolor = orange] [
      move-to one-of patches in-radius vision-range with [pcolor = orange]
      set state "hiding"
...
to zombie-follow-horde
  ...
  if [target-human] of following-zombie != nobody [
    ...
    if distance target-human < vision-range [
      set state "chasing"
      set following-zombie nobody
      set chase-timer 0  ; Shorter timer for followers
    ]
  ]
end
