...
to setup-humans
  create-humans initial-human-population [
    ...
    set color green
    set size 1.5
    move-to one-of patches with [abs(pxcor) < 5 and abs(pycor) < 5 and not any? humans-here]
...
to setup-zombies
  create-zombies initial-zombie-population [
    ...
    set speed 0.4 + random-float 0.4   ; Zombies are slower than humans
...
to human-behavior
  ...
  if (hunger-level > 70 or energy < 40) and resource-inventory > 0 [
    eat-food
  ]
...
to eat-food
  if resource-inventory > 0 [
    ...
    let food-consumed min (list 15 resource-inventory)
...
to handle-intragroup-resource-competition [target-resource competitors]
  ...
  (ifelse
    ...
    current-resource-pressure < 30 and cooperation-level > 55 and hunger-level < 30 [
      ...
      if random 100 < 50 [
        ...
        ; NEW: Share resources with nearby struggling agents
        let recipient one-of humans in-radius 2 with [resource-inventory < 15 and self != myself]
        if recipient != nobody [
          let amount-shared 10
          if resource-inventory >= amount-shared [
            set resource-inventory resource-inventory - amount-shared
            ask recipient [
              set resource-inventory resource-inventory + amount-shared
            ]
          ]
